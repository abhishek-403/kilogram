      ...
      <div className="pt-10 gap-4 flex flex-col">
        {
          allTab.map((item, index) => {
            return (
              <Tabs icon={item.icon} key={index} route={item.route} title={item.title}/>
            );
          })
        }
      </div>
...
function Tabs({icon,title,route}) {
  ...
  return (
    <div onClick={()=>navigate(`${route}`)} className="cursor-pointer flex-row pl-3 pr-28 py-2 flex gap-5 hover:bg-[#1A1A1A] rounded-md align-middle">
